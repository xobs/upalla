...
// ---- Frame processing helper (shared by both chains) ----
...
pub fn run_filter(
    ...
    // Detect BlackHole for dual-chain routing.
    // Playback:  BlackHole input  → denoise → speakers
    // Recording: mic              → denoise → BlackHole output
    ...
    // ---- Playback chain (system audio → speakers) ----
    let (pb_stream_in, mut pb_in_cons) = if let Some(ref bh) = bh_input {
        let (s, c) = create_input(bh, &config)?;
        (Some(s), Some(c))
    } else {
        (None, None)
    };
    ...
    let (mut rec_stream_in, mut rec_in_cons) = create_input(&default_input, &config)?;
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            match cmd {
                Cmd::SwitchModel(new_model) => {
                    ...
                    pb_denoiser = Denoiser::new(&new_model, 2)?;
                    rec_denoiser = Denoiser::new(&new_model, 2)?;
                }
                ...
                Cmd::SetSink(name) => {
                    // Changes playback output (speakers for system audio chain)
                ...
                Cmd::SetSource(name) => {
                    // Changes recording input (mic for the recording chain)
