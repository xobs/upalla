...
fn main() -> Result<()> {
    ...
    if sr as usize != model.sr {
        log::warn!("Sample rate {sr} != {} Hz.", model.sr);
    }
    ...
    let process_channel = |model: &mut DfTract, input: &[f32]| -> Result<Vec<f32>> {
        ...
        for i in 0..n_frames {
            ...
            let enh_view = ArrayViewMut2::from_shape((1, hop).f(), &mut enhanced)?;
    ...
    // Create fresh model for right channel
